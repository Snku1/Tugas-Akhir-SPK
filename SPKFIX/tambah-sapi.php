...
if(isset($_POST['submit'])):	
	...
	if(empty($errors)):
		...
		$handle = $pdo->prepare('INSERT INTO sapi (no_kalung, ciri_khas, tanggal_input) VALUES (:no_kalung, :ciri_khas, :tanggal_input)');
		$handle->execute( array(
			'no_kalung' => $no_kalung,
			'ciri_khas' => $ciri_khas,
			'tanggal_input' => date('Y-m-d')
		) );
		$sukses = "sapi no. <strong>{$no_kalung}</strong> berhasil dimasukkan.";
		$id_sapi = $pdo->lastInsertId();
		
		// Jika ada kriteria yang diinputkan:
		if(!empty($kriteria)):
			foreach($kriteria as $id_kriteria => $nilai):
				$handle = $pdo->prepare('INSERT INTO nilai_sapi (id_sapi, id_kriteria, nilai) VALUES (:id_sapi, :id_kriteria, :nilai)');
				$handle->execute( array(
					'id_sapi' => $id_sapi,
		...
		redirect_to('list-sapi.php?status=sukses-baru');		
	...
	<div class="container clearfix">
		...
		<div class="main-content the-content">
			...
			<?php endif; ?>			
				...
				<form action="tambah-sapi.php" method="post">
					...
					<div class="field-wrap clearfix">
						<button type="submit" name="submit" value="submit" class="button">Tambah Sapi</button>
					</div>
				</form>
